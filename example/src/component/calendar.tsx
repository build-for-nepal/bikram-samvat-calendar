...
export default function CalendarComponent() {
    ...
    const renderFormats = () => {
        ...
        const formats = [
            { label: 'YYYY/MM/DD', value: nepaliDate.format('YYYY/MM/DD'), method: "format('YYYY/MM/DD')" },
            { label: 'YYYY MM DD', value: nepaliDate.format('YYYY MM DD'), method: "format('YYYY MM DD')" },
            { label: 'YYYY', value: nepaliDate.format('YYYY'), method: "format('YYYY')" },
            { label: 'ddd DD, MMMM YYYY', value: nepaliDate.format('ddd DD, MMMM YYYY'), method: "format('ddd DD, MMMM YYYY')" },
            { label: 'Today is ddd DD, MMMM YYYY', value: nepaliDate.format('To\\day is ddd DD, MMMM YYYY'), method: "format('To\\day is ddd DD, MMMM YYYY')" },
            { label: 'DD/MM/YYYY (np)', value: nepaliDate.format('DD/MM/YYYY', 'np'), method: "format('DD/MM/YYYY', 'np')" },
            { label: 'dd (np)', value: nepaliDate.format('dd', 'np'), method: "format('dd', 'np')" },
            { label: 'ddd DD, MMMM YYYY (np)', value: nepaliDate.format('ddd DD, MMMM YYYY', 'np'), method: "format('ddd DD, MMMM YYYY', 'np')" },
        ];
            ...
            <div className="formats-container">
                ...
                {formats.map(({ label, value, method }) => (
                    <div className="format-item" key={label}>
                        ...
                        <small> Method: <code>{method}</code></small>
                        <button className="copy-button" onClick={() => copyToClipboard(value)}>Copy</button>
                    </div>
                ))}
            </div>
        );
